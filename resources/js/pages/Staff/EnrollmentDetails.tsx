...
export default function EnrollmentDetails({ learner }: EnrollmentDetailsProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
      ...
      const birthplaceParts = [
        learner.birthplace_city_municipality,
        learner.birthplace_province,
        learner.birthplace_region
    ].filter(Boolean);

    const fullBirthplace = birthplaceParts.length > 0 ? birthplaceParts.join(', ') : 'N/A';
            ...
            <div className="px-8 py-8 bg-gradient-to-br from-slate-50 to-gray-100 min-h-screen">
                <div className="max-w-7xl mx-auto">
                            ...
                            <div className="p-6 space-y-4">
                                <div className="grid grid-cols-1 gap-4">
                                    ...
                                    <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                            ...
                                            <span className="text-gray-900 text-right">{fullBirthplace}</span>
                                        </div>
                                    <div className="flex items-center justify-between py-2 border-b border-gray-100">
                    ...
                    <div className="mt-8 flex justify-center space-x-4">
                         <a
                    href={route('staff.enrollment.downloadPdf', learner.learner_id)}
                    target="_blank" // Opens the PDF in a new tab
                    className="px-6 py-3 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-lg hover:from-emerald-700 hover:to-green-700 transition-all duration-300 shadow-lg hover:shadow-xl font-semibold inline-flex items-center"
                >
                    ...
                    Download PDF
                        ...
                        <Link
                                // Ensure 'learner' and 'learner.learner_id' exist before using them
                                href={learner?.learner_id ? route('staff.enrollment.edit', learner.learner_id) : '#'}
                                as="button"
                                // Disable the link if there's no ID, preventing errors
                                disabled={!learner?.learner_id}
                                className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-lg hover:shadow-xl font-semibold inline-flex items-center"
                            >
                                <svg className="w-5 h-5 inline mr-2" fill="currentColor" viewBox="0 0 20 20">
