...
const REQUIRED_FIELDS_BY_STEP = {
    0: ['last_name', 'first_name', 'gender', 'civil_status', 'birth_date', 'age', 'email', 'nationality'],
...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ programs = [] }) => {
    ...
    // Custom hooks
    const { regions, provinces, cities, barangays, selectedCodes, setSelectedCodes } = useAddressData();
    ...
    ), [data, allErrors, handleInputChange, nationalities]);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 bg-dark p-6 sm:p-10 rounded-xl shadow-2xl max-w-4xl mx-auto my-8 font-inter">
           ...
           // RegistrationForms.tsx - Continuation from where it left off
